...
`run.sh` is now the one-command setup path on the target M4: it validates macOS arm64 prerequisites, syncs the application and local MLX runtime, structurally verifies the expected `ltx-2-mlx` package and console-script layout, and detects the q8 model pack. The default managed runtime checkout is self-healing: if an older or incompatible checkout is already present under `data/runtime/ltx-2-mlx`, `run.sh` replaces it with the configured `dgrauet/ltx-2-mlx` repository instead of repeatedly failing on stale runtime metadata. When the model pack is absent it interactively offers to download `dgrauet/ltx-2.3-mlx-q8` into the configured local model directory; `LTX_AUTO_DOWNLOAD_MODEL=yes|no` provides non-interactive control. Custom `LTX_RUNTIME_DIR` checkouts are never silently replaced. Model download now uses the current Hugging Face `hf` CLI through `uvx hf download`, replacing the deprecated `huggingface-cli` invocation. The model source and runtime checkout are environment-overridable, and all model/runtime assets remain uncommitted local state.

The dashboard media workflow now runs without a whole-screen blocking loader. Starting background media generation leaves the rest of the page usable, disables only the media-generation action to prevent duplicate submissions, and shows persistent inline progress with queued/running/completed stage text. The progress bar becomes an indeterminate animated bar while an individual scene is undergoing expensive temporal generation because the durable job API currently reports completed scenes rather than fine-grained model inference percentages; scene-count progress is shown when scenes complete. Terminal states refresh project artifacts and final-video readiness without restoring a global modal overlay.
