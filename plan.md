...
`run.sh` is now the one-command setup path on the target M4: it validates macOS arm64 prerequisites, syncs the application and local MLX runtime, verifies the `ltx-2-mlx` CLI, and detects the q8 model pack. When the model pack is absent it interactively offers to download `dgrauet/ltx-2.3-mlx-q8` into the configured local model directory; `LTX_AUTO_DOWNLOAD_MODEL=yes|no` provides non-interactive control. The model source and runtime checkout are environment-overridable, and all model/runtime assets remain uncommitted local state.
