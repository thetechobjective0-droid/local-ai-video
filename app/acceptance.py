    ...
    *,
    ...
    if project_id is not None and execute_media:
        ...
        try:
            ...
            video_result, _ = generate_scene_video(
                video_provider,
                store,
                project_id,
                scene,
            ...
            add(
                "video_generation",
                video_result.path.is_file() and video_result.path.stat().st_size > 0,
                f"{video_result.path.name}; {video_result.width}x{video_result.height}@{video_result.fps}",
