...
def regenerate_scene(project_id: str, scene_id: str, stage: str = typer.Option("all", "--stage", click_type=typer.Choice(["image", "audio", "video", "all"])), seed: int | None = typer.Option(None, "--seed"), config: Path | None = typer.Option(None, "--config", exists=True)) -> None:
    ...
    if stage in {"image", "all"}:
        ...
        image_provider, _ = _image_provider_and_store(config)
        result = generate_scene_image_with_recovery(image_provider, store, project_uuid, scene, model=config_data.image.model_path.name, width=config_data.image.width, height=config_data.image.height, steps=config_data.image.steps, guidance_scale=config_data.image.guidance_scale, seed=seed)
        scene = result.scene
    ...
    if stage in {"video", "all"}:
        ...
        try:
            result = generate_scene_video_with_recovery(video_provider, store, project_uuid, scene, width=config_data.video.width, height=config_data.video.height, fps=config_data.video.fps, seed=seed)
            scene = result.scene
...
def acceptance(project_id: str | None = typer.Option(None, "--project-id"), no_media: bool = typer.Option(False, "--no-media"), report_path: Path | None = typer.Option(None, "--report", help="Write JSON report to this path."), config: Path | None = typer.Option(None, "--config", exists=True)) -> None:
    ...
    report = run_acceptance(load_config(config), UUID(project_id) if project_id else None, execute_media=not no_media)
    output = write_report(report, report_path or load_config(config).storage.root / "acceptance-report.json")
