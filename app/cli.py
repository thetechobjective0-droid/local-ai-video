...
def _image_provider_and_store(
    ...
    ), FilesystemStore(config.storage.root)
...
def _tts_provider_and_store(config_path: Path | None) -> tuple[MacOSTTSProvider, FilesystemStore]:
    ...
    return MacOSTTSProvider(sample_rate=config.tts.sample_rate), FilesystemStore(
        config.storage.root
    )
...
def create(
    ...
    directory = create_plan(
        ...
        top_p=config_data.llm.top_p,
        top_k=config_data.llm.top_k,
        quality_profile=config_data.runtime.profile,
    )
...
@app.command()
def resume(
    project_id: str, config: Path | None = typer.Option(None, "--config", exists=True)
) -> None:
...
def generate_scene(
    ...
    artifact, _ = generate_scene_image(
        ...
        UUID(project_id),
        scene,
        model=config_data.image.model_path.name,
        width=config_data.image.width,
        height=config_data.image.height,
        steps=config_data.image.steps,
        guidance_scale=config_data.image.guidance_scale,
        seed=seed,
    )
...
def generate_audio(
    ...
    artifact, _ = generate_scene_audio(
        ...
        store,
        UUID(project_id),
        _load_scene(store, UUID(project_id), UUID(scene_id)),
        voice=config_data.tts.voice,
        rate=config_data.tts.rate,
    )
...
def generate_media(
    ...
    results = generate_project_media(
        store,
        UUID(project_id),
        _load_scenes(store, UUID(project_id)),
        video_provider=video_provider,
        image_provider=image_provider,
        image_model=config_data.image.model_path.name,
        image_width=config_data.image.width,
        image_height=config_data.image.height,
        image_steps=config_data.image.steps,
        image_guidance_scale=config_data.image.guidance_scale,
        video_capability=get_provider_capabilities(config_data.video.provider).video,
        fallback_provider=build_video_fallback(config_data),
        width=config_data.video.width,
        height=config_data.video.height,
        fps=config_data.video.fps,
    )
...
def regenerate_scene(
    ...
    stage: str = typer.Option(
        "all", "--stage", click_type=click.Choice(["image", "audio", "video", "all"])
    ),
    seed: int | None = typer.Option(None, "--seed"),
    config: Path | None = typer.Option(None, "--config", exists=True),
) -> None:
...
def subtitles(
    ...
    outputs = build_subtitles(
        store, UUID(project_id), _load_scenes(store, UUID(project_id)), max_chars=max_chars
    )
...
def timeline(
    ...
    result = build_timeline(
        store, store.load_project(project_uuid), _load_scenes(store, project_uuid)
    )
...
def acceptance(
    ...
    report_path: Path | None = typer.Option(
        None, "--report", help="Write JSON report to this path."
    ),
    config: Path | None = typer.Option(None, "--config", exists=True),
) -> None:
    ...
    report = run_acceptance(
        config_data, UUID(project_id) if project_id else None, execute_media=not no_media
    )
    output = write_report(
        report, report_path or config_data.storage.root / "acceptance-report.json"
    )
