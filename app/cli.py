...
def regenerate_scene(
    ...
    stage: str = typer.Option(
        "all", "--stage", click_type=typer.Choice(["image", "audio", "video", "all"])
    ),
    seed: int | None = typer.Option(None, "--seed"),
    config: Path | None = typer.Option(None, "--config", exists=True),
) -> None:
...
def acceptance(
    ...
    report = run_acceptance(
        load_config(config), UUID(project_id) if project_id else None, execute_media=not no_media
    )
    output = write_report(
        report, report_path or load_config(config).storage.root / "acceptance-report.json"
    )
