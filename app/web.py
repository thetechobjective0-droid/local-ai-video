    ...
    for suffix in ("image", "audio", "video"):
        try:
            ...
            if exc.status_code == 404:
                continue
            raise
        artifacts[suffix] = {**artifact.model_dump(mode="json"), "url": f"/api/projects/{project_id}/scenes/{scene_id}/{suffix}"}
...
def regenerate(project_id: UUID, scene_id: UUID, request: RegenerateRequest) -> dict[str, Any]:
    ...
    try:
        if request.stage == "image":
            ...
            _, scene = generate_scene_audio(audio_provider, store, project_id, scene, voice=config.tts.voice, rate=config.tts.rate)
...
def final_video_status(project_id: UUID) -> dict[str, Any]:
    ...
    return {"ready": True, "url": f"/api/projects/{project_id}/video", "size": path.stat().st_size}
