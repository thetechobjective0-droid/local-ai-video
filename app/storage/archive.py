...
from app.storage.integrity import build_integrity_manifest, verify_integrity_manifest
...
def export_project(store: FilesystemStore, project_id: UUID | str, destination: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="video-export-") as temp_dir:
        ...
        temp_root.mkdir()
        manifest_path = build_integrity_manifest(source, temp_root / "integrity-manifest.json")
        ...
        for item in source.iterdir():
            if item.name in {"integrity-manifest.json"}:
                continue
        ...
        build_integrity_manifest(temp_root, temp_root / "integrity-manifest.json")
    ...
    with tempfile.TemporaryDirectory(prefix="video-import-") as temp_dir:
        ...
        with tarfile.open(archive_path, "r:gz") as archive:
            for member in archive.getmembers():
                ...
                if extraction != target and extraction not in target.parents:
        ...
        if not verify_integrity_manifest(root, manifest):
            raise ValueError("project integrity verification failed")
        destination.parent.mkdir(parents=True, exist_ok=True)
