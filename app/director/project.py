    ...
    if project.status == ProjectStatus.STORYBOARD_READY:
        ...
        if storyboard is not None:
            try:
                ...
                logger.warning("[director] persisted storyboard invalid for target duration project=%s error=%s; invalidating dependent artifacts", project.id, exc)
                ...
                if not brief_matches_duration:
                    logger.warning("[director] persisted brief duration=%ss target=%ss; restarting planning from brief", brief.duration_seconds if brief else None, project.duration_seconds)
                    start_from = "brief"
                else:
