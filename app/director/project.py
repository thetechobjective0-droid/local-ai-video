...
def _persist_subtitles(directory: Path, storyboard: Storyboard) -> None:
    ...
    for number, scene in enumerate(entries, start=1):
        ...
        srt_parts.extend(
            [str(number), f"{_subtitle_timestamp(start)} --> {_subtitle_timestamp(end)}", text, ""]
        )
        vtt_parts.extend(
            [
...
def _persist_storyboard_outputs(
    ...
    logger.info(
        "[director] materializing storyboard project=%s scenes=%s",
        project.id,
    ...
    for scene in sorted(storyboard.scenes, key=lambda item: item.index):
        ...
        logger.info(
            "[director] scene manifest persisted project=%s index=%s id=%s",
            project.id,
            scene.index,
            scene.id,
        )
    timeline_scenes = [
        TimelineScene(
            ...
            subtitle_start_seconds=scene.subtitle_range[0] if scene.subtitle_range else None,
            subtitle_end_seconds=scene.subtitle_range[1] if scene.subtitle_range else None,
...
def resume_plan(
    ...
    *,
    ...
    """Resume planning while invalidating all artifacts tied to an old duration contract."""
    directory = store.project_dir(project_id)
    project = store.load_project(project_id)
    logger.info(
        "[director] resume project=%s current_status=%s target_duration=%ss",
        project.id,
    ...
    brief_matches_duration = (
        brief is not None and abs(brief.duration_seconds - project.duration_seconds) < 1e-6
    )
    ...
    if project.status == ProjectStatus.STORYBOARD_READY:
        ...
        if storyboard is not None:
            try:
                ...
                logger.warning(
                    "[director] persisted storyboard invalid for target duration project=%s error=%s; invalidating dependent artifacts",
                    project.id,
                    exc,
                )
                # The storyboard was generated from the persisted brief/script. If either
                # carries the old duration, restart at brief so the duration contract is
                # propagated through every Director stage.
                if not brief_matches_duration:
                    logger.warning(
                        "[director] persisted brief duration=%ss target=%ss; restarting planning from brief",
                        brief.duration_seconds if brief else None,
                        project.duration_seconds,
                    )
                    start_from = "brief"
                else:
...
def _run_plan(
    ...
    *,
    ...
    logger.info(
        "[director] pipeline START project=%s start_from=%s target_duration=%ss",
        project.id,
        start_from,
        project.duration_seconds,
    )
    try:
        ...
        if (
            start_from == "brief"
            or brief is None
            or abs(brief.duration_seconds - project.duration_seconds) >= 1e-6
        ):
            brief = _run_stage(
                store,
                directory,
            ...
            logger.info(
                "[director] brief persisted project=%s title=%r duration=%ss",
                project.id,
                brief.title,
