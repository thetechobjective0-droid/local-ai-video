...
class MediaJobManager:
    ...
    def _run(self, job_id: UUID) -> None:
        ...
        try:
            ...
            logger.info("[media] initializing image provider model=%s", config.image.model_path)
            ...
            generate_project_media(
                ...
                image_model=config.image.model_path.name, image_width=config.image.width, image_height=config.image.height,
                image_steps=config.image.steps, image_guidance_scale=config.image.guidance_scale,
                video_capability=capability, fallback_provider=build_video_fallback(config),
                width=config.video.width, height=config.video.height, fps=config.video.fps,
